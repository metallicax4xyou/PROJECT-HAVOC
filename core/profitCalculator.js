...
// --- VERSION v2.3 ---
// Adds intermediate sim amount and gas limit to finalTradeData.
...
class ProfitCalculator {
     ...
     * and an estimateGas validity check. Adds intermediate sim amount and gas limit to tradeData.
     */
    async evaluateOpportunity(opportunity, signerAddress) {
        ...
        // --- 1. Validate & Setup ---
        // *** TODO: Generalize this beyond 2-hop spatial if needed ***
        if (opportunity?.type !== 'spatial' || opportunity.path?.length !== 2 || !opportunity.tokenIn || !opportunity.tokenIntermediate) { return { isProfitable: false, reason: "Malformed structure (currently only supports 2-hop spatial)" }; }
        ...
        const initialToken = this.config.TOKENS[opportunity.tokenIn.symbol]; // Use symbol lookup
        ...
        if (!initialToken || !intermediateToken || !finalToken || initialToken.symbol !== finalToken.symbol) { return { isProfitable: false, reason: `Token mismatch/missing in config.TOKENS` }; }
        ...
        let amountIntermediate = 0n; // Initialize
        const sim1Result = await this.swapSimulator.simulateSwap(poolBuyState, initialToken, amountInStart);
        if (!sim1Result.success || !sim1Result.amountOut || sim1Result.amountOut <= 0n) { return { isProfitable: false, reason: `Leg 1 Sim Fail: ${sim1Result.error || 'Zero output'}` }; }
        amountIntermediate = sim1Result.amountOut; // Store the result of the first hop
        ...
        if (!sim2Result.success || !sim2Result.amountOut || sim2Result.amountOut <= 0n) { return { isProfitable: false, reason: `Leg 2 Sim Fail: ${sim2Result.error || 'Zero output'}` }; }
        ...
        if (grossProfitWei_InitialToken <= 0n) { return { isProfitable: false, netProfitNativeWei: null, reason: "Negative gross profit (sim)" }; }
        ...
        // *** Store the gas limit used ***
        const gasLimitEstimate = gasCostDetails.pathGasLimit; // Assuming this is the key from GasEstimator v1.4
        if (!gasLimitEstimate || gasLimitEstimate <= 0n) {
            // Should not happen if totalCostWei is valid, but check anyway
            return { isProfitable: false, netProfitNativeWei: null, reason: "Invalid gas limit in gasCostDetails", tradeData: null };
        }
        ...
        try {
            ...
            const bufferMultiplier = 10000n - (this.profitBufferPercent * 100n); if (bufferMultiplier <= 0n) throw new Error("Invalid profit buffer.");
            ...
            if (isProfitableAfterThreshold) {
                ...
                // --- Construct finalTradeData with ADDED fields ---
                const finalTradeData = {
                    ...opportunity, // Original opportunity data (type, path, etc.)
                    amountIn: amountInStart.toString(), // Initial input amount
                    intermediateAmountOut: amountIntermediate.toString(), // <<< ADDED: Amount after hop 1
                    amountOut: finalAmountOut.toString(), // Final output amount
                    profitAmount: grossProfitWei_InitialToken.toString(), // Gross profit (in tokenIn)
                    profitAmountNativeWei: grossProfitNativeWei.toString(), // Gross profit (native wei)
                    gasCostNativeWei: gasCostNativeWei.toString(), // Estimated gas cost (native wei)
                    netProfitNativeWei: netProfitNativeWei.toString(), // Net profit (native wei)
                    gasEstimate: gasLimitEstimate.toString(), // <<< ADDED: Gas LIMIT estimate (BigInt as string)
                    profitPercentage: profitPercentage, // Calculated profit percentage
                    thresholdNativeWei: thresholdNativeWei.toString(), // Profit threshold used (native wei)
                    timestamp: Date.now() // Timestamp of calculation
                };
                // --- ---
            ...
            logger.error(`${logPrefix} Error during final eval: ${evalError.message}`);
            return { isProfitable: false, netProfitNativeWei, reason: `Eval error: ${evalError.message}`, tradeData: null };
        }
    }
}
