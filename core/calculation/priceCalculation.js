...
// --- VERSION v1.5 --- Corrected price calculation logic to use token decimals for scaling.
 ...
 * Returns price of token0 in terms of token1 (T0/T1) scaled by 10^decimals1.
 * This means the price represents how many smallest units of token1 you get for 1 standard unit of token0.
 * Returns null on error.
 * Formula: price = ((sqrtPriceX96 / 2**96)^2) * (10^decimals1 / 10^decimals0)
 * Scaled Price (T0/T1, 10^decimals1) = price * 10^decimals1
 * = (((sqrtPriceX96 * sqrtPriceX96) / Q192) * ((10n ** decimals1) / (10n ** decimals0))) * (10n ** decimals1) -- This is wrong
 *
 * Correct formula for Price T0/T1 scaled by 10^decimals1:
 * Price Standard (T0/T1) = (sqrtPriceX96 / Q96)^2 * (10^decimals1 / 10^decimals0)
 * Scaled Price (T0/T1, 10^decimals1) = Price Standard * 10^decimals1
 * = (((sqrtPriceX96 * sqrtPriceX96) / Q192) * (10^decimals1 / 10^decimals0)) * 10^decimals1
 * Integer arithmetic: (sqrtPriceX96 * sqrtPriceX96 * (10n ** decimals1) * (10n ** decimals1)) / (Q192 * (10n ** decimals0)) -- Still looks complex
 *
 ...
 *
 * Scaled Price T0/T1 (10^decimals1) = Price T0/T1 Standard * 10^decimals1
 * = ((Q192 * 10^decimals1) / (sqrtPriceX96 * sqrtPriceX96 * 10^decimals0)) * 10^decimals1
 * Integer arithmetic: (Q192 * (10n ** decimals1) * (10n ** decimals1)) / (sqrtPriceX96 * sqrtPriceX96 * (10n ** decimals0))
 *
 * This still seems prone to precision loss. Let's stick to the T1/T0 scaled price and invert it outside.
 * The previous `calculateV3PriceT1_T0_scaled` was correct for T1/T0 scaled by 1e18.
 * The issue is the display/interpretation.
 *
 * Let's revert calculateV3PriceT1_T0_scaled to its previous correct state and keep the function name.
 */
function calculateV3PriceT1_T0_scaled(poolState) { // Reverted function name and logic
    ...
    if (poolState.token0?.decimals === undefined || poolState.token0?.decimals === null ||
        ...
        BigInt(poolState.token0.decimals) < 0n || BigInt(poolState.token1.decimals) < 0n) {
    ...
    try {
        ...
        const numeratorScaled = numerator * scale0 * PRICE_SCALE;
 ...
 * Scaled Price (T0/T1, 10^decimals1) = price * 10^decimals1
 * = (reserve1 / reserve0) * (10^decimals0 / 10^decimals1) * 10^decimals1
 * = (reserve1 * 10^decimals0) / reserve0
 * This price represents how many *smallest* units of token1 you get for 1 *standard* unit of token0.
 *
 * Let's calculate Price T0/T1 scaled by PRICE_SCALE (1e18) instead.
 * Scaled Price T0/T1 (1e18) = Price T0/T1 Standard * PRICE_SCALE
 * Price T0/T1 Standard = (reserve1 / 10^decimals1) / (reserve0 / 10^decimals0) = (reserve1 * 10^decimals0) / (reserve0 * 10^decimals1)
 * Scaled Price T0/T1 (1e18) = ((reserve1 * 10^decimals0) / (reserve0 * 10^decimals1)) * PRICE_SCALE
 * Integer arithmetic: (reserve1 * (10n ** decimals0) * PRICE_SCALE) / (reserve0 * (10n ** decimals1)) -- This matches the previous code.
 */
...
function calculateDodoPrice(poolState) {
    ...
    try {
         ...
         if (token0.address.toLowerCase() === baseToken.address.toLowerCase() &&
             ...
             // Case 2: T1 is Base, T0 is Quote. We need Price(Quote/Base) scaled by PRICE_SCALE.
             // Price(Quote/Base standard) = 1 / Price(Base/Quote standard) = scaleQuote / queryAmountOutWei
