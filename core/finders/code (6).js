...
// --- VERSION v1.13 --- Adjusted log levels for less verbose per-cycle output.
...
class SpatialFinder {
    ...
    constructor(config) {
        ...
        logger.info(`[SpatialFinder v1.13] Initialized. Min Net BIPS: ${this.minNetPriceDiffBips}, Max Diff BIPS: ${this.maxReasonablePriceDiffBips}, Sim Amounts Loaded: ${Object.keys(this.simulationInputAmounts).length} (Filters DODO Quote Sell)`); // Updated version log
    }
    ...
    _createOpportunity(poolSwapT1toT0, poolSwapT0toT1, canonicalKey, tokenBorrowedOrRepaid, tokenIntermediate) {
        ...
        const logPrefix = `[SF._createOpp ${canonicalKey} v1.13]`; // Updated version log
            ...
            if (!baseTokenAddress) {
                ...
                logger.error(`${logPrefix} Cannot determine base token address for DODO pool ${poolSwapT1toT0.address}. Cannot filter quote sell reliably. Skipping opportunity.`);
                 ...
                 if (tokenBorrowedOrRepaid.address.toLowerCase() !== baseTokenAddress.toLowerCase()) {
                     ...
                     // Keeping this specific filter message at WARN level as it indicates a known limitation/filter
