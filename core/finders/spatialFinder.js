...
const MIN_NET_PRICE_DIFFERENCE_BIPS = 0n; // TEMPORARY: Set back after test!
...
class SpatialFinder {
    constructor(config) {
        ...
        logger.info(`[SpatialFinder v1.28] Initialized (Fixed poolState, Logging Moved).`);
    }
        ...
        for (const [canonicalKey, poolAddressesSet] of this.pairRegistry.entries()) {
            ...
            for (let i = 0; i < poolsWithPrices.length; i++) {
                for (let j = i + 1; j < poolsWithPrices.length; j++) {
                    ...
                    // Sanity Check
                    const rawPriceDiff = rawPriceA > rawPriceB ? rawPriceA - rawPriceB : rawPriceB - rawPriceA; // Corrected typo here too
                    const minRawPrice = rawPriceA < rawPriceB ? rawPriceA : rawPriceB; if (minRawPrice === 0n) continue; const rawDiffBips = (rawPriceDiff * BASIS_POINTS_DENOMINATOR) / minRawPrice; if (rawDiffBips > MAX_REASONABLE_PRICE_DIFF_BIPS) { logger.warn(`[SF] Implausible RAW diff (${rawDiffBips} bips). Skip.`); continue; }
                    ...
                    // *** MOVED DETAILED LOGGING HERE (AFTER VARIABLES DEFINED) ***
                    ...
                    const comparisonBvsA = effectiveSellPriceB > rawPriceA; // Define comparison results
