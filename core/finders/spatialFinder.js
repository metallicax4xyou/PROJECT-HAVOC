...
class SpatialFinder {
    constructor(config) {
        ...
        logger.info(`[SpatialFinder v1.31] Initialized.`); // Version reflects logic state
    }
        ...
        for (const [canonicalKey, poolAddressesSet] of this.pairRegistry.entries()) {
            ...
            for (let i = 0; i < poolsWithPrices.length; i++) {
                for (let j = i + 1; j < poolsWithPrices.length; j++) {
                    const poolA = poolsWithPrices[i]; const poolB = poolsWithPrices[j]; const rawPriceA = poolA.price0_1_scaled; const rawPriceB = poolB.price0_1_scaled;
                    // Sanity Check
                    ...
                    // logger.debug(`[SF Compare] ...`);

                    let poolBuy = null, poolSell = null, netDiffBips = 0n;
                    if (comparisonBvsA) { netDiffBips = ((effectiveSellPriceB - rawPriceA) * BASIS_POINTS_DENOMINATOR) / rawPriceA; if (netDiffBips >= MIN_NET_PRICE_DIFFERENCE_BIPS) { poolBuy = poolA; poolSell = poolB; } } // Use restored threshold
                    else if (comparisonAvsB) { netDiffBips = ((effectiveSellPriceA - rawPriceB) * BASIS_POINTS_DENOMINATOR) / rawPriceB; if (netDiffBips >= MIN_NET_PRICE_DIFFERENCE_BIPS) { poolBuy = poolB; poolSell = poolA; } } // Use restored threshold
