...
const MIN_NET_PRICE_DIFFERENCE_BIPS = 0n; // Original: 5n (0.05%). Set back after test!
// *** --- ***
...
class SpatialFinder {
    ...
    findArbitrage(poolStates) {
            ...
            for (let i = 0; i < poolsWithPrices.length; i++) {
                for (let j = i + 1; j < poolsWithPrices.length; j++) {
                    ...
                    if (effectiveSellPriceB > rawPriceA) { // Sell on B, Buy on A
                        ...
                        if (netDiffBips >= MIN_NET_PRICE_DIFFERENCE_BIPS) { // Using the potentially modified 0n threshold here
                    ...
                    if (poolBuy && poolSell) {
                        ...
                        const buyPriceFormatted = ethers.formatUnits(poolBuy.price0_1_scaled, 18);
                        const sellPriceFormatted = ethers.formatUnits(poolSell === poolA ? effectiveSellPriceA : effectiveSellPriceB, 18);
        ...
        logger.info(`[SpatialFinder] Finished scan. Found ${opportunities.length} potential spatial opportunities (using NET threshold: ${MIN_NET_PRICE_DIFFERENCE_BIPS}).`); // Log threshold used
    ...
    /**
     ...
     _createOpportunity(poolBuy, poolSell, canonicalKey) {
         ...
         const extractSimState = (pool) => ({
              ...
              sqrtPriceX96: pool.sqrtPriceX96, reserve0: pool.reserve0, reserve1: pool.reserve1,
              token0: pool.token0, token1: pool.token1, baseTokenSymbol: pool.baseTokenSymbol, tick: pool.tick
         });
             ...
             path: [
                 {
                     ...
                     priceScaled: poolBuy.price0_1_scaled.toString(), poolState: extractSimState(poolBuy)
                 },
                 {
                     ...
                     priceScaled: poolSell.price0_1_scaled.toString(), poolState: extractSimState(poolSell)
                 }
             ],
