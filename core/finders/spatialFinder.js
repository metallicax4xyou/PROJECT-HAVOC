...
class SpatialFinder {
    constructor(config) {
        ...
        logger.info(`[SpatialFinder v1.28] Initialized (Fixed poolState for DODO sim).`);
    }
    ...
    findArbitrage(poolStates) { /* ... main loop unchanged ... */
        ...
        for (const [canonicalKey, poolAddressesSet] of this.pairRegistry.entries()) {
            ...
            // logger.debug(`[SpatialFinder] Comparing ${poolsWithPrices.length} pools for pair ${canonicalKey}...`); // Can uncomment for verbose pair logging
            ...
            for (let i = 0; i < poolsWithPrices.length; i++) {
                for (let j = i + 1; j < poolsWithPrices.length; j++) {
                    ...
                    // Detailed Logging
                    // logger.debug(`[SF Compare] ${poolA.token0?.symbol}/${poolA.token1?.symbol} | ${poolA.dexType}(${poolA.address.substring(0,4)}.. Fee:${feeA_bips}) vs ${poolB.dexType}(${poolB.address.substring(0,4)}.. Fee:${feeB_bips})`); logger.debug(`  Raw Prices (0->1): A=${ethers.formatEther(rawPriceA)} | B=${ethers.formatEther(rawPriceB)}`); logger.debug(`  Eff.Sell Prices:   A=${ethers.formatEther(effectiveSellPriceA)} | B=${ethers.formatEther(effectiveSellPriceB)}`); const comparisonBvsA = effectiveSellPriceB > rawPriceA; const comparisonAvsB = effectiveSellPriceA > rawPriceB; logger.debug(`  Check (Sell B > Raw A): ${comparisonBvsA} | Check (Sell A > Raw B): ${comparisonAvsB}`);
    ...
    /**
     ...
     _createOpportunity(poolBuy, poolSell, canonicalKey) {
        ...
        // *** UPDATED extractSimState TO INCLUDE baseTokenSymbol ***
        const extractSimState = (pool) => ({
             ...
             token0: pool.token0, // Pass actual token objects
             token1: pool.token1,
             baseTokenSymbol: pool.baseTokenSymbol, // *** ADDED THIS LINE ***
             tick: pool.tick,
             // Include DODO query results if needed directly by simulator (optional)
             // queryAmountOutWei: pool.queryAmountOutWei,
             // queryBaseToken: pool.queryBaseToken, // Caution: might be large objects
        ...
        return {
            ...
            path: [
                { // Buy Leg
                    dex: poolBuy.dexType, address: poolBuy.address, pairSymbols: getPairSymbols(poolBuy),
                    action: 'buy', tokenInSymbol: quoteToken.symbol, tokenOutSymbol: targetToken.symbol,
                    priceScaled: poolBuy.price0_1_scaled.toString(),
                    poolState: extractSimState(poolBuy) // Include extracted state
                },
                { // Sell Leg
                    dex: poolSell.dexType, address: poolSell.address, pairSymbols: getPairSymbols(poolSell),
                    action: 'sell', tokenInSymbol: targetToken.symbol, tokenOutSymbol: quoteToken.symbol,
                    priceScaled: poolSell.price0_1_scaled.toString(),
                    poolState: extractSimState(poolSell) // Include extracted state
                }
            ],
