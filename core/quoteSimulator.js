...
async function simulateArbitrage(provider, opportunity) {
    ...
    try {
        ...
        // Access the quotient (JSBI object)
        const quotientJSBI = amountInForSimulation.quotient;

        // Check if quotientJSBI is a valid JSBI object
        if (!(quotientJSBI instanceof JSBI)) {
            logger.error(`[Sim Arb] Validation FAIL: Returned CurrencyAmount's quotient is not a JSBI object. Type: ${typeof quotientJSBI}. Value:`, quotientJSBI);
            return null;
        }
        logger.debug(`[Sim Arb] Validation PASS: Result is CurrencyAmount with JSBI quotient.`);

        // --->>> Convert JSBI quotient to native BigInt <<<---
        simAmountBigInt = BigInt(quotientJSBI.toString());
        logger.debug(`[Sim Arb] Converted JSBI quotient to native BigInt: ${simAmountBigInt}`);

        // Proceed with native BigInt
    ...
    // --- Actual Simulation Logic (Needs updated quotient handling) ---
    try {
        ...
        // --->>> Convert final amount's JSBI quotient to native BigInt <<<---
        const finalAmountReceived_Sim_JSBI = amountOutSwap2_Sim.quotient;
        if (!(finalAmountReceived_Sim_JSBI instanceof JSBI)) {
             logger.error("[Simulator] Final simulated amount quotient is not JSBI!", finalAmountReceived_Sim_JSBI);
             throw new ArbitrageError('Internal simulation error: Invalid type for finalAmountReceived_Sim_JSBI.', 'SIMULATION_ERROR');
        }
        const finalAmountReceived_Sim_Raw = BigInt(finalAmountReceived_Sim_JSBI.toString());
        ...
        // --->>> Extrapolation uses native BigInts (simAmountBigInt, borrowAmount, finalAmountReceived_Sim_Raw) <<<---
        logger.debug("[Simulator] ---> Calculating Estimated Profit...");
        let finalAmountReceived_Actual_Estimated = 0n;
         if (simAmountBigInt > 0n && typeof finalAmountReceived_Sim_Raw === 'bigint') { // Check should pass now
             finalAmountReceived_Actual_Estimated = (finalAmountReceived_Sim_Raw * borrowAmount) / simAmountBigInt;
         } else {
              logger.error(`[Simulator] Invalid inputs for profit extrapolation: simAmountBigInt=${simAmountBigInt}, finalAmountReceived_Sim_Raw=${finalAmountReceived_Sim_Raw}`);
...
// --- getMinimumAmountOut FUNCTION (Needs JSBI handling) ---
function getMinimumAmountOut(trade, slippageToleranceBps) {
    if (!trade) return 0n; // Return native 0n
    const slippageTolerance = new Percent(slippageToleranceBps, 10000);
    const amountOut = trade.minimumAmountOut(slippageTolerance); // This returns a CurrencyAmount

    // --->>> Convert JSBI quotient to native BigInt <<<---
    if (amountOut && amountOut.quotient instanceof JSBI) {
        return BigInt(amountOut.quotient.toString());
    } else {
        logger.warn("[Simulator] getMinimumAmountOut received invalid trade or amountOut object.", { trade, amountOut });
        return 0n; // Return native 0n on error
    }
}
...
// Simplified error handler for brevity in the catch block
const genericErrorHandler = (error, context) => {
     if (error instanceof ArbitrageError) { handleError(error, context); }
     else { handleError(error, `${context} (Unexpected)`); }
     logger.error(`[Simulator] Error during ${context}: ${error.message}`);
     return null;
}
// Replace catch blocks with: } catch (error) { return genericErrorHandler(error, 'Context String'); }
