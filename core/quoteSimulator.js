...
// TickLens Contract Info & simulateSingleTradeSDK (Keep forced logging from previous version)
...
async function simulateSingleTradeSDK( provider, poolAddress, poolForTrade, tokenIn, tokenOut, amountIn ) { logger.info(`[SimSDK ENTRY] Simulating on pool ${poolAddress} (${tokenIn.symbol} -> ${tokenOut.symbol})`); if (!provider) { logger.error('[SimSDK] Provider instance is required.'); return null; } if (!ethers.isAddress(poolAddress)) { logger.error(`[SimSDK] Invalid poolAddress received: ${poolAddress}`); return null; } if (!(poolForTrade instanceof Pool) || typeof poolForTrade.tickSpacing !== 'number') { logger.error('[SimSDK] Invalid poolForTrade object.', { poolForTrade }); return null; } if (!amountIn || !(amountIn.quotient instanceof JSBI) || amountIn.quotient.raw.equalTo(JSBI.BigInt(0))) { logger.warn(`[SimSDK] Invalid or zero input amount.`); return null; } const tickSpacing = poolForTrade.tickSpacing; try { const tickLensContract = new ethers.Contract(TICK_LENS_ADDRESS, TICK_LENS_ABI, provider); const tickBitmapIndex = 0; logger.info(`[SimSDK] Fetching ticks for ${poolAddress} (Index ${tickBitmapIndex})...`); let populatedTicks = []; try { populatedTicks = await tickLensContract.getPopulatedTicksInWord(poolAddress, tickBitmapIndex); logger.info(`[SimSDK] Fetched ${populatedTicks?.length ?? 0} populated ticks for ${poolAddress}.`); } catch (tickFetchError) { logger.warn(`[SimSDK] Error fetching ticks for pool ${poolAddress}: ${tickFetchError.message}.`); handleError(tickFetchError, `TickLens Fetch (${poolAddress})`); return null; } const tickDataProvider = new TickListDataProvider(populatedTicks || [], tickSpacing); logger.info(`[SimSDK] Creating Route for ${poolAddress}...`); const route = new Route([poolForTrade], tokenIn, tokenOut); logger.info(`[SimSDK] Attempting Trade.fromRoute for ${poolAddress}...`); const trade = await Trade.fromRoute(route, amountIn, TradeType.EXACT_INPUT, { tickDataProvider }); logger.info(`[SimSDK] Trade.fromRoute successful for ${poolAddress}.`); if (!trade || !trade.outputAmount || !(trade.outputAmount.quotient instanceof JSBI)) { logger.warn(`[SimSDK] Trade simulation for ${poolAddress} returned invalid trade object or outputAmount/quotient.`); return null; } logger.info(`[SimSDK EXIT] Simulation SUCCESS for pool ${poolAddress}.`); return trade; } catch (error) { logger.error(`[SimSDK ERROR] Pool ${poolAddress} (${tokenIn.symbol}->${tokenOut.symbol}): ${error.message}`); handleError(error, `simulateSingleTradeSDK (${tokenIn.symbol} -> ${tokenOut.symbol}) Pool: ${poolAddress}`); return null; } }
...
// --- simulateArbitrage FUNCTION with DETAILED RETURN LOGGING ---
async function simulateArbitrage(provider, opportunity) {
    // --->>> Log Entry <<<---
    logger.debug(`[Sim Arb ENTRY] Group: ${opportunity?.groupName}`);

    if (!provider) { logger.error("[Sim Arb EXIT] Provider missing"); return null; }
    if (!opportunity || !opportunity.swapPoolInfo?.sdkPool || !opportunity.sdkTokenBorrowed || !opportunity.sdkTokenIntermediate || !opportunity.borrowAmount) { logger.warn("[Sim Arb EXIT] Invalid opportunity structure"); return null; }
    if (typeof opportunity.sdkTokenBorrowed.decimals !== 'number' || typeof opportunity.sdkTokenIntermediate.decimals !== 'number') { logger.error("[Sim Arb EXIT] Invalid token decimals"); return null; }
    ...
    if (!ethers.isAddress(swapPoolAddress)) { logger.error(`[Sim Arb EXIT] Invalid swapPoolInfo.address found: ${swapPoolAddress}`); return null; }
    ...
    try {
        ...
        if (!(amountInForSimulation instanceof CurrencyAmount)) { logger.error(`[Sim Arb EXIT] Validation FAIL: calculateDynamicSimAmount did not return a CurrencyAmount object. Got:`, amountInForSimulation); return null; }
        const quotientJSBI = amountInForSimulation.quotient;
        if (!(quotientJSBI instanceof JSBI)) { logger.error(`[Sim Arb EXIT] Validation FAIL: Returned CurrencyAmount's quotient is not a JSBI object. Type: ${typeof quotientJSBI}. Value:`, quotientJSBI); return null; }
        logger.debug(`[Sim Arb] Validation PASS: Result is CurrencyAmount with JSBI quotient.`);
        simAmountBigInt = BigInt(quotientJSBI.toString()); logger.debug(`[Sim Arb] Converted JSBI quotient to native BigInt: ${simAmountBigInt}`);
        ...
        logger.error(`[Sim Arb EXIT] Error during pre-simulation checks: ${debugError.message}`, debugError);
        handleError(debugError, `QuoteSimulator.PreSimulation (${opportunity?.groupName})`);
        // --->>> Explicit return null <<<---
        logger.info("[Sim Arb RETURN] Returning null due to pre-simulation error.");
        return null;
    }
    ...
    try {
        ...
        logger.info("[Sim Arb] ---> Simulating Swap 1...");
        ...
        if (!trade1) {
            logger.warn(`[Sim Arb] Simulation FAIL Swap 1 returned null.`);
             // --->>> Explicit return null <<<---
             logger.info("[Sim Arb RETURN] Returning null because trade1 was null.");
            return null;
        }
        ...
        logger.info(`[Sim Arb] Sim Swap 1 OK.`);

        logger.info("[Sim Arb] ---> Simulating Swap 2...");
        ...
        if (!trade2) {
             logger.warn(`[Sim Arb] Simulation FAIL Swap 2 returned null.`);
             // --->>> Explicit return null <<<---
             logger.info("[Sim Arb RETURN] Returning null because trade2 was null.");
            return null;
        }
        ...
        logger.info(`[Sim Arb] Sim Swap 2 OK.`);

        // --->>> Profit Calculation <<<---
        const finalAmountReceived_Sim_JSBI = amountOutSwap2_Sim.quotient; if (!(finalAmountReceived_Sim_JSBI instanceof JSBI)) { throw new ArbitrageError('Internal simulation error: Invalid type for finalAmountReceived_Sim_JSBI.', 'SIMULATION_ERROR'); } const finalAmountReceived_Sim_Raw = BigInt(finalAmountReceived_Sim_JSBI.toString()); logger.debug(`[Sim Arb] Final Amount Raw (native): ${finalAmountReceived_Sim_Raw}`);
        logger.info("[Sim Arb] ---> Calculating Estimated Profit..."); let finalAmountReceived_Actual_Estimated = 0n; if (simAmountBigInt > 0n && typeof finalAmountReceived_Sim_Raw === 'bigint') { finalAmountReceived_Actual_Estimated = (finalAmountReceived_Sim_Raw * borrowAmount) / simAmountBigInt; } else { throw new ArbitrageError('Internal simulation error: Invalid inputs for profit extrapolation.', 'SIMULATION_ERROR'); } logger.log(`[Simulator] Est. Final Amount Received: ${ethers.formatUnits(finalAmountReceived_Actual_Estimated, sdkTokenBorrowed.decimals)} ${symbolToUse}`); logger.log(`[Simulator] Required Repayment: ${ethers.formatUnits(requiredRepaymentAmount, sdkTokenBorrowed.decimals)} ${symbolToUse}`);

        if (finalAmountReceived_Actual_Estimated <= requiredRepaymentAmount) {
            logger.log(`[Simulator] Gross Profit Check FAIL.`);
            // --->>> Explicit return null <<<---
            logger.info("[Sim Arb RETURN] Returning null because gross profit check failed.");
            return null;
        }
        const grossProfit = finalAmountReceived_Actual_Estimated - requiredRepaymentAmount;
        logger.log(`[Simulator] Est. Gross Profit Found: ${ethers.formatUnits(grossProfit, sdkTokenBorrowed.decimals)} ${symbolToUse}`);
        logger.info("[Sim Arb] Simulation and Profit Calc SUCCESS.");

        // --->>> SUCCESS RETURN <<<---
        const successResult = { finalAmountReceived: finalAmountReceived_Actual_Estimated, requiredRepayment: requiredRepaymentAmount, grossProfit: grossProfit, simulatedAmountIn: simAmountBigInt, trade1: trade1, trade2: trade2 };
        logger.info("[Sim Arb RETURN] Returning SUCCESS result object:", successResult);
        return successResult;

    } catch (error) {
        const context = `QuoteSimulator.simulateArbitrage (${opportunity?.groupName})`;
        if (error instanceof ArbitrageError) { handleError(error, context); }
        else { handleError(error, `${context} (Unexpected)`); }
        logger.error(`[Sim Arb ERROR] Error during simulation/calculation: ${error.message}`);
        // --->>> Explicit return null <<<---
        logger.info("[Sim Arb RETURN] Returning null due to caught error during simulation/calculation.");
        return null;
    }
} // End simulateArbitrage
