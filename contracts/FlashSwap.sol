...
// --- Contract Definition ---
// --- VERSION v3.14 --- Fixed Aave interface methods & callback profit calculation order, temporarily disabled DODO execution
contract FlashSwap is IUniswapV3FlashCallback, IFlashLoanReceiver, ReentrancyGuard {
    ...
    address public constant TREASURY = 0x50d3414C549a0A9Df8d29eD5872FDaEf97d6748d; // Address for the 30% tithe
...
// --- END OF PART 1 ---
    ...
    function uniswapV3FlashCallback( uint256 fee0, uint256 fee1, bytes calldata data ) external override nonReentrant {
        ...
        // Distribute Net Profit (30% Treasury, 70% Owner)
        if (netProfit > 0) {
            uint titheAmount = (netProfit * 30) / 100;
            uint ownerAmount = netProfit - titheAmount;
...
// --- END OF PART 2 ---
    ...
    function executeOperation(
        ...
        // Profit calculation happens BEFORE approval/distribution
...
// --- END OF PART 3 ---
    ...
    function _executeTriangularSwaps( address _tokenA, uint _amountA, bytes memory _params ) internal returns (uint finalAmount) {
        ...
        finalAmount = _executeSingleV3Swap(
            ...
            pathParams.fee3,
            amountC,
            pathParams.amountOutMinimumFinal
        );
    ...
    function _executeSwapPath(SwapStep[] memory _path) internal returns (uint finalAmount) {
        ...
        for (uint i = 0; i < _path.length; i++) {
            ...
            if (step.dexType == DEX_TYPE_UNISWAP_V3) {
                ...
                // --- DODO Swaps (Currently Disabled for Compilation) ---
                revert("FS:DODO_TEMP_DISABLED");
                /*
                // To re-enable, update IDODOV1V2Pool interface or hardcode base/quote if needed
                IDODOV1V2Pool dodoPool = IDODOV1V2Pool(step.pool);
                address baseToken = dodoPool.baseToken(); // <-- Requires baseToken() in interface
                address quoteToken = dodoPool.quoteToken(); // <-- Requires quoteToken() in interface

                if (step.tokenIn == baseToken && step.tokenOut == quoteToken) {
                    try dodoPool.sellBase(amountIn, step.minOut, address(this), block.timestamp + DEADLINE_OFFSET) returns (uint256 receivedAmount) { amountOut = receivedAmount; emit SwapExecuted(i+1, DEX_TYPE_DODO, step.tokenIn, step.tokenOut, amountIn, amountOut); } catch Error(string memory reason) { revert(string(abi.encodePacked("FS:D", _numToString(i+1), "SF:", reason))); } catch { revert(string(abi.encodePacked("FS:D", _numToString(i+1), "SFL"))); }
                } else if (step.tokenIn == quoteToken && step.tokenOut == baseToken) {
                     try dodoPool.buyBase(amountIn, step.minOut, address(this), block.timestamp + DEADLINE_OFFSET) returns (uint256 receivedAmount) { amountOut = receivedAmount; emit SwapExecuted(i+1, DEX_TYPE_DODO, step.tokenIn, step.tokenOut, amountIn, amountOut); } catch Error(string memory reason) { revert(string(abi.encodePacked("FS:D", _numToString(i+1), "BF:", reason))); } catch { revert(string(abi.encodePacked("FS:D", _numToString(i+1), "BFL"))); }
                } else {
...
// --- END OF PART 4 ---
    // --- Helper Functions ---
    // Approves spender if current allowance is less than max uint256.
    ...
    // --- External Functions ---
    // Initiates a Uniswap V3 flash loan
    ...
    // Initiates an Aave V3 flash loan
