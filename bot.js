...
// Fetches pool data sequentially.
...
function sqrtPriceX96ToPrice(sqrtPriceX96, decimals0, decimals1) {
    if (!sqrtPriceX96 || sqrtPriceX96 === 0n) {
        console.warn("   sqrtPriceX96ToPrice received zero sqrtPriceX96!"); return NaN;
    }
    const Q96 = 2n**96n; const Q192 = Q96**2n;
    const numerator = sqrtPriceX96 * sqrtPriceX96 * (10n**BigInt(decimals0));
    const denominator = Q192 * (10n**BigInt(decimals1));
    const multiplier = 10n**18n; const priceBigInt = (numerator * multiplier) / denominator;
...
async function checkArbitrage() {
    ...
    try {
        // --- 1. Get Pool Data & Determine Order (SEQUENTIAL FETCH) ---
        let slot0_005, slot0_030, token0_pool005, token1_pool005;
        try {
            // console.log("   Fetching slot0_005..."); // Reduce noise unless debugging fetch
            ...
            // console.log("   ...Fetch complete."); // Reduce noise
        } catch (fetchError) {
        ...
        else { return; } // Exit if prices equal
