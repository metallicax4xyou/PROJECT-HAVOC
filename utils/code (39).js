...
// --- VERSION v1.16 --- Added temporary logging for encoding parameters.
...
class GasEstimator {
    ...
    async _encodeMinimalCalldataForEstimate(opportunity, providerType) {
        ...
        try {
            ...
            if (providerType === 'UNIV3') {
                 ...
                 // --- ADDED TEMPORARY LOGGING HERE ---
                 logger.debug(`${logPrefix} Encoding params for AbiCoder: typeString="${buildResult.typeString}", params=`, JSON.stringify(buildResult.params, (k, v) => typeof v === 'bigint' ? v.toString() : v, 2)); // Log the params object
                 ...
                 const encodedParamsBytes = ethers.AbiCoder.defaultAbiCoder().encode([buildResult.typeString], [buildResult.params]); // <-- ERROR LINE (Count=4, Expected=6)

                 // Args for initiateUniswapV3FlashLoan(address poolAddress, uint256 amount0, uint256 amount1, bytes params)
                 const args = [borrowPoolAddress, amount0, amount1, encodedParamsBytes]; // Pass encoded ArbParams struct
                 ...
                 const encodedArbParamsBytes = ethers.AbiCoder.defaultAbiCoder().encode([buildResult.typeString], [needsReEncoding ? adjustedParams : buildResult.params]); // <-- ERROR LINE (Count=4, Expected=6)
    ...
    async estimateTxGasCost(opportunity, walletSignerAddress) { // Renamed from signerAddress for clarity
        ...
        if (!this.config.FLASH_SWAP_CONTRACT_ADDRESS || !ethers.isAddress(this.config.FLASH_SWAP_CONTRACT_ADDRESS)) {
             ...
             logger.error(`${logPrefix} ${errorMsg}`);
             return { pathGasLimit, effectiveGasPrice: 0n, totalCostWei: 0n, estimateGasSuccess: false, errorMessage: errorMsg };
        }
